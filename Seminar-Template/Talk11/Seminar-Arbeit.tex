...
\label{sec:bmf_comparison}
	...
	\subsection{Ecosystem Business Model Framework}
		...
		One of the `Value Drivers' behind the business idea of `FlexSpace' is sustainability which is addressed by using less office space per employee. Also, the employee satisfaction is expected to increase due to the fact that employees can choose their own desk place flexibly after the introduction of the system. The availability of usage data 

		The FlexSpace Employee, Beacons, Beacon Manufacturer, Beacon Installation Partner, Managers, Employees, Office Usage Application, Internal IT department make up the `Value Nodes'. Note that there are two autonomous nodes with the beacons and the application.
