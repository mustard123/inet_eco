...
\label{sec:bmf_comparison}
	...
	\subsection{Ecosystem Business Model Framework}
		...
		One of the `Value Drivers' behind the business idea of `FlexSpace' is sustainability which is reached by using less office space per employee thanks to the beacon solution. Also, the employee satisfaction is expected to increase due to the fact that employees can choose their own desk place flexibly after the introduction of the system. The availability of usage data can is a starting point to turn that data to value for the customer company. 

		The FlexSpace employees, beacons, beacon manufacturer, beacon installation partners, the customer's managers and employees, the application as well as the customer company's internal IT department make up the `Value Nodes'. Note that there are two autonomous nodes, namely the beacons and the application. Some are individuals (e.g. employees) and others are organizations (e.g. internal IT department)

		`Value Exchanges' take place between between all kinds of nodes as indicated in Figure~\ref{fig:use_case_westerlund}. Only a couple of these exchanges are applicably for `Value Extract'. Namely the installation process, the licensing of software usage, the work of installation partners as well as training given to the customer are monetizable. It is, for example, hard to image a monetization for the value exchange between the employee and the beacon.
