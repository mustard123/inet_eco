...
\label{sec:bmf_available}
	...
	\subsection{Adapted Business Model Canvas for IoT (Dijkman et al., 2015)}
		...
		\todo{TODO: wrapup}
		\todo{TODO: explain most significant building block types in detail if space is available, else better wrapup}
...
\label{sec:bmf_comparison}
	...
	\todo{describe rest of building blocks}
	...
	When we apply Chan's framework to our fictional company, figure \ref{use case chan} shows the possible result. The emphasis on the mutual benefits for all collaborators becomes evident. Thanks to the beacons at each desk where the employees log themselves in, the target company now knows about the used capacity of workplaces. They can which employees come on premise to work and who rather works from home. The software provided can measure changes in utilization over the year. Predictions can be made, flexible allocations can be made to adjust the varying need of workplaces over the year. Employees profit by being free to choose where they want to work and never running into an occupied workplace.
